...
- **Docker Non-Root Exception.** `docker/Dockerfile` and `docker/Dockerfile.dev` run the container as root instead of dropping to a non-root user, per the documented-exception clause in AI.md PART 26 ("Dockerfile Rules"). Reason: the binary binds the privileged internal port 80, and it must create/chown `/config` and `/data` on first run (per PART 26's zero-config Docker Volume Mapping contract) since Docker auto-creates root-owned, unwritable bind-mount directories on the host when they don't already exist. AI.md PART 26's own entrypoint.sh contract explicitly forbids the entrypoint from creating directories, setting permissions, or creating a user/group — that setup is reserved for the binary itself — so the classic root-entrypoint-chowns-then-drops-privilege pattern (gosu/su-exec) is not available. Empirically verified: building with `addgroup`/`adduser`/`setcap cap_net_bind_service`/`USER app` succeeds, but the container then fails at startup with `permission denied` writing `/data/search.pid` against a freshly auto-created bind mount. Trade-off: the runtime container has root privileges instead of the ecosystem-standard non-root user. Mitigation: `CGO_ENABLED=0` static binary, minimal Alpine base, no shell-exposed attack surface beyond the entrypoint/binary themselves.
...
##### 3. Cross-Device Sync (Portable, accountless)
Per AI.md "Cross-device preference sync": only `theme` and `lang` are portable across devices. This is stateless — the server never stores anything; the exported URL/code *is* the preference values, decoded, validated, and written back as cookies on import.

**Use Cases:**
- **Sync URL**: `https://search.example.com/server/preferences/import?theme=dark&lang=fr`
- **Short Code**: base64url form of the same query string, for manual retyping
- **QR Code**: Scan to apply theme/language on another device

**Workflow:**
1. Settings page → Cross-Device Sync section → "Copy Preferences"
2. Fetches `GET /server/preferences/export`, which reads the current `theme`/`lang` cookies and returns the full URL, short code, and a QR link
3. On another device, paste the URL/code into "Paste a Sync Code or URL" → "Apply", which navigates to `GET /server/preferences/import`
4. Import validates each value against its allowlist, sets the matching cookies, and redirects — no account, no server-side row

**Note:** The broader `?prefs=` URL parameter (category, safe search, results per page, new-tab behavior, pagination mode, keyboard shortcuts) is still read client-side on page load for bookmarklet/manual-link use, but it no longer has a dedicated "Generate Link" button in Settings — the Shareable Preference Link section was merged into Cross-Device Sync since both existed to move preferences to a new device/browser.
...
│ │ Sync URL:                                           │ │
│ │ ┌───────────────────────────────────────────────┐   │ │
│ │ │ https://example.com/server/preferences/import? │   │ │
│ │ │ theme=dark&lang=fr                              │   │ │
│ │ └───────────────────────────────────────────────┘   │ │
│ │ Sync Code:                                          │ │
│ │ ┌───────────────────────────────────────────────┐   │ │
│ │ │ dGhlbWU9ZGFyayZsYW5nPWZy                       │   │ │
│ │ └───────────────────────────────────────────────┘   │ │
│ │ [Copy Preferences] [QR Code]                        │ │
│ │                                                     │ │
│ │ Paste a Sync Code or URL:                           │ │
│ │ ┌───────────────────────────────────────────────┐   │ │
│ │ │                                                 │   │ │
│ │ └───────────────────────────────────────────────┘   │ │
│ │ [Apply]                                             │ │
