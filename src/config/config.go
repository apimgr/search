...
// Per AI.md PART 25: LDFLAGS must include Version, CommitID, BuildEpoch, OfficialSite
// BuildDate is NOT embedded - it is derived from BuildEpoch in init()
var (
	Version  = "dev"
	CommitID = "unknown"
...
type ServerConfig struct {
	...
	// Notifications — per AI.md PART 17: webui toasts (client-side) and email
	// (SMTP), including per-event email enable/disable switches.
...
// NotificationsConfig represents server.notifications per AI.md PART 17:
// public WebUI toasts (client-side, visitor-facing) and email (SMTP),
// including per-event email enable/disable switches.
type NotificationsConfig struct {
	WebUI WebUINotifyConfig `yaml:"webui"`
	Email EmailConfig       `yaml:"email"`
}

// WebUINotifyConfig represents client-side toast notification defaults.
// Per AI.md PART 17 "Sane Defaults": position top-right, duration 5s.
...
type EmailConfig struct {
	...
	// Per-event email settings (override defaults) — AI.md PART 17
	Events EmailEventsConfig `yaml:"events"`
}

// EmailEventsConfig represents per-event email enable/disable switches.
// Per AI.md PART 17 "Configuration" → server.notifications.email.events.
...
type GeoIPConfig struct {
	...
	// Named operator-authored country lists (name -> codes), for reuse in
	// deny_countries/allow_countries; empty by default, never pre-populated
	// with a bundled regulatory list per AI.md PART 19 "Country Blocking Presets"
...
func addConfigComments(node *yaml.Node) {
	...
	serverSubComments := map[string]string{
		...
		"notifications":    "WebUI toast and email/SMTP notification settings",
...
func (c *Config) ApplyRuntimeEnv(env *EnvConfig) {
	...
	if env.SMTPHost != "" {
		c.Server.Notifications.Email.SMTP.Host = env.SMTPHost
	}

	// SMTP_PORT — SMTP server port
	if env.SMTPPort != 0 {
		c.Server.Notifications.Email.SMTP.Port = env.SMTPPort
	}

	// SMTP_USERNAME — SMTP authentication username
	if env.SMTPUsername != "" {
		c.Server.Notifications.Email.SMTP.Username = env.SMTPUsername
	}

	// SMTP_PASSWORD — SMTP authentication password
	if env.SMTPPassword != "" {
		c.Server.Notifications.Email.SMTP.Password = env.SMTPPassword
	}

	// SMTP_FROM_NAME — Sender name
	if env.SMTPFromName != "" {
		c.Server.Notifications.Email.From.Name = env.SMTPFromName
	}

	// SMTP_FROM_EMAIL — Sender email
	if env.SMTPFromEmail != "" {
		c.Server.Notifications.Email.From.Email = env.SMTPFromEmail
	}

	// SMTP_TLS — TLS mode: auto, starttls, tls, none
	if env.SMTPTLS != "" {
		c.Server.Notifications.Email.SMTP.TLS = env.SMTPTLS
	}
}
