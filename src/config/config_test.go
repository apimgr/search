...
func TestApplyEnvSMTP(t *testing.T) {
	...
	if cfg.Server.Notifications.Email.SMTP.Host != "smtp.example.com" {
		t.Errorf("SMTP Host not applied, got %q", cfg.Server.Notifications.Email.SMTP.Host)
	}
	if cfg.Server.Notifications.Email.SMTP.Port != 587 {
		t.Errorf("SMTP Port not applied, got %d", cfg.Server.Notifications.Email.SMTP.Port)
	}
}
...
func TestEmailConfigDefaults(t *testing.T) {
	...
	if cfg.Server.Notifications.Email.SMTP.Port != 587 {
		t.Errorf("Email.SMTP.Port = %d, want 587", cfg.Server.Notifications.Email.SMTP.Port)
	}
	if cfg.Server.Notifications.Email.SMTP.TLS != "auto" {
		t.Errorf("Email.SMTP.TLS = %q, want %q", cfg.Server.Notifications.Email.SMTP.TLS, "auto")
	}
}
...
func TestDefaultEngines(t *testing.T) {
	...
	expectedEngines := []string{"google", "duckduckgo", "bing", "brave"}
