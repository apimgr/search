...
// Per AI.md PART 23/25: System user keyed on the frozen {internal_name}
// identity, with home in /var/lib/{internal_org}/{internal_name}.
func (sm *ServiceManager) ensureSystemUser() error {
	userName := config.InternalName
	homeDir := filepath.Join("/var/lib", config.InternalOrg, config.InternalName)
...
exec chpst -u {{.User}}:{{.Group}} /usr/local/bin/search 2>&1
`
...
func (sm *ServiceManager) installRunit() error {
	...
	data := map[string]string{
		"User":      config.InternalName,
		"Group":     config.InternalName,
...
// openrcTemplate returns the OpenRC init script.
// Per AI.md PART 23/25: Service starts as root, binary drops to the
// {internal_name} system user/group after port binding.
func openrcTemplate() string {
	owner := config.InternalName + ":" + config.InternalName
	libDir := filepath.Join("/var/lib", config.InternalOrg, config.InternalName)
	logDir := filepath.Join("/var/log", config.InternalOrg, config.InternalName)
	cacheDir := filepath.Join("/var/cache", config.InternalOrg, config.InternalName)
...
output_log="%s/stdout.log"
error_log="%s/stderr.log"
...
start_pre() {
	checkpath --directory --owner %s %s
	checkpath --directory --owner %s %s
	checkpath --directory --owner %s %s
}
`, logDir, logDir, owner, libDir, owner, logDir, owner, cacheDir)
}
