...
# Build via the Makefile's `dev` target — never invoke `go build` directly
# (per AI.md PART 25: all toolchain commands run through Docker via the
# Makefile's GO_DOCKER wrapper, never a raw `docker run ... go build`).
# `make dev` builds to a fresh temp dir and prints "Built: <dir>/<binary>".
echo "Building via 'make dev' (server + CLI, if present)..."
DEV_OUTPUT=$(make dev 2>&1)
echo "$DEV_OUTPUT"
BUILT_LINE=$(echo "$DEV_OUTPUT" | grep -- '^Built: ' | tail -1)
if [ -z "$BUILT_LINE" ]; then
    echo "FAILED: could not find 'Built: <path>' line in 'make dev' output"
    exit 1
fi
BUILD_DIR=$(dirname "${BUILT_LINE#Built: }")
